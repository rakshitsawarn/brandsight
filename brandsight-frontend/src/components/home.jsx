...
const Home = () => {
  ...
  const analyzeBrand = async () => {
    ...
    const mapsRegex =
      /^https:\/\/www\.google\.[a-z.]+\/maps\/place\/[^\/]+\/@[\d.,z]+\/data=/;
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/nlpApi/analyze",
        {
          UID: user.uid,
          brandURL,
          brandURLType,
  ...
  const ViewHistory = async () => {
    ...
    try {
      const historyResponse = await axios.post(
        "http://localhost:5000/api/users/getHistory",
        {
          UID: user.uid,
        },
        {
  ...
  const DownloadReport = async () => {
    ...
    doc.text(`Date: ${formattedDate}`, 105, cursorY, { align: "center" });
